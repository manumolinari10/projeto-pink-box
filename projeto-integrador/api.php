...
// 1. Tabela padrão (mude para uma tabela que exista no seu banco)
$tabela_solicitada = $_GET['tabela'] ?? 'customers';

// 2. Coloque as tabelas que você tem no banco aqui
...
// 3. COLE A SUA CHAVE ANON AQUI DE NOVO
...
// --- O MOTOR PROFISSIONAL (cURL) ---
$ch = curl_init(); // Inicia o cURL

curl_setopt($ch, CURLOPT_URL, $url_supabase);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);

// Esta linha mágica diz para o Laragon ignorar a falta de certificados locais (Resolve o bloqueio do Windows)
curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false); 

// Adiciona a nossa chave na porta
...
$codigo_http = curl_getinfo($ch, CURLINFO_HTTP_CODE); // Pega a resposta do Supabase (ex: 200, 404, 401)
...
if ($resultado === false || $codigo_http != 200) {
    ...
    echo json_encode([
        "status" => "erro", 
        "mensagem_php" => $erro_curl, // Se for erro de rede, mostra aqui
        "codigo_supabase" => $codigo_http, // Se for erro de permissão, mostra o número aqui
        "detalhe_supabase" => json_decode($resultado) // O Supabase costuma explicar o erro
    ]);
} else {
